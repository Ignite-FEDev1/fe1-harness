...
export default function OrchestratorPage() {
  ...
  useEffect(() => {
    setLoading(true);
    fetch('/api/pipelines/orchestrator')
      .then(r => r.json())
      .then(data => {
  ...
  const handleSave = async () => {
    setSaving(true);
    setSaveMsg('');
    await fetch('/api/pipelines/orchestrator', {
      method: 'PUT',
    ...
    <div className="flex flex-col" style={{ height: '100%', overflow: 'hidden' }}>
      ...
      <div className="flex items-center justify-between flex-shrink-0" style={{ padding: '10px 20px', borderBottom: '1px solid var(--border-dim)', background: 'var(--bg-base)' }}>
        <div className="flex items-center gap-2">
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '12px', fontWeight: 700, letterSpacing: '0.1em', color: 'var(--text-muted)', background: 'rgba(255,165,0,0.1)', border: '1px solid rgba(255,165,0,0.3)', borderRadius: '3px', padding: '2px 7px' }}>SYSTEM</span>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '12px', fontWeight: 700, color: '#fb923c' }}>orchestrator.md</span>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '12px', color: 'var(--text-muted)' }}>· 파이프라인 실행 엔진 (모든 파이프라인 공유)</span>
        </div>
        <div className="flex items-center gap-2">
